...
void ow_write_bit(uint8_t bit) {
	...
	#define OW_WRB_DELAY_SHORT_1 7 //8 // short delay
	#define OW_WRB_DELAY_LONG_1 60 //72 // long delay
...
uint8_t ow_read_bit(void) {
	...
	#define OW_RDB_DELAY_01 5 //1
	#define OW_RDB_DELAY_02 1 //1
          ...
          nrf_drv_pwm_simple_playback(&m_pwm0, &m_pwm_seq, 1, NRFX_PWM_FLAG_STOP);
			...
			for (i=0;i<8;i++) {
				...
				for (j=0;j<8;j++) {
					...
					t >>= 1;
					nrf_delay_ms(10);//delay1ms(10); // 10ms write bit 
				} // end for(j)
				ptr++;
			} // end for(i)
			// wait more
			nrf_delay_ms(100);//delay1ms(100);
			// verify written rom
           nrf_drv_pwm_simple_playback(&m_pwm0, &m_pwm_seq, 1, NRF_DRV_PWM_FLAG_LOOP);             
			if (ow_reset() == 0) {
           nrf_drv_pwm_simple_playback(&m_pwm0, &m_pwm_seq, 1, NRFX_PWM_FLAG_STOP);
